...
<div class="container">
    <div class="card">
        ...
        <p><strong>Tên người đặt:</strong> <c:out value="${customer != null && customer.fullname != null && customer.fullname != '' ? customer.fullname : order.username}"/></p>
        <p><strong>Người nhận:</strong> ${order.receiver_name}</p>
        <p><strong>Số điện thoại người nhận:</strong> ${order.receiver_phone}</p>
        ...
        <p><strong>Giá sau khi trừ voucher:</strong> <fmt:formatNumber value="${order.totalmoney}" type="number" /> đ</p>
        <c:if test="${voucherDiscount > 0}">
            <p><strong>Giảm voucher:</strong> <fmt:formatNumber value="${voucherDiscount}" type="number" /> đ</p>
        </c:if>
        <p><strong>Tổng trước voucher:</strong> <fmt:formatNumber value="${originalTotal}" type="number" /> đ</p>
    </div>
